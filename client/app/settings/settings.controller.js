...
angular.module('clickeatApp').controller('SettingsController', function($scope, Auth, $timeout, $http, Upload) {
   ...
   // $scope.user.nhsSys = [];
    ...
    function updateSelectedList() {
        ...
        $timeout(function() {
            for (var i = 0; i < $scope.user.nhsSys.length; i++) {
                for (var j = 0; j < $scope.nhsdata.length; j++) {
                    if ($scope.user.nhsSys[i].id == $scope.nhsdata[j].id) {
    ...
    $scope.edit = function(index) {
        $scope.currentRef = $scope.user.references[index];
    ...
    $scope.add = function() {
        if ($scope.currentRef.name && $scope.currentRef.mobile && $scope.currentRef.id) {
            ...
            if ($scope.currentIndex >= 0) {
                $scope.user.references[$scope.currentIndex] = $scope.currentRef;
            } else {
                $scope.user.references.push($scope.currentRef);
            }
            $scope.currentRef = {};
        } else {
