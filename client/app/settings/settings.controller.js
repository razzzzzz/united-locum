...
angular.module('clickeatApp').controller('SettingsController', function($scope,Auth,$timeout, $http, Upload){
    ...
    $scope.triggerFileClick = function($event){
        $timeout(function(){
            angular.element($event.currentTarget).closest("tr").find("input:file").trigger("click");
        });
    };

  $scope.sendEmail =function(file,toMail){

    /*$http.get('/api/profiles/simple').then(function(err,respo){
      console.log('asdasd',err,respo);
    });*/
    Upload.upload({
      url: '/api/profiles/sendmail', //webAPI exposed to upload the file
      data:{file:file,'mailusername': toMail} //pass file as data, should be user ng-model
    }).then(function (resp) { //upload function returns a promise
      alert('mail sent successfully to '+toMail);
    });
  };
       ...
       $scope.uploadFile = function(fname, model,GMC_form){ //function to call on form submit
         console.log('sss',fname, model,GMC_form);
           if(!model){
